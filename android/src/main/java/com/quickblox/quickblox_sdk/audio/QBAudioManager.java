package com.quickblox.quickblox_sdk.audio;
...
public class QBAudioManager {
    ...
    public QBAudioManager.AudioDevice appRTCAudioDeviceFromValue(int value) {
        QBAudioManager.AudioDevice audioDevice = null;
        if (value == AudioOutput.BLUETOOTH.value) {
            audioDevice = QBAudioManager.AudioDevice.BLUETOOTH;
        }
        if (value == AudioOutput.EARSPEAKER.value) {
            audioDevice = QBAudioManager.AudioDevice.EARPIECE;
        }
        if (value == AudioOutput.HEADPHONES.value) {
            audioDevice = QBAudioManager.AudioDevice.WIRED_HEADSET;
        }
        if (value == AudioOutput.LOUDSPEAKER.value) {
            audioDevice = QBAudioManager.AudioDevice.SPEAKER_PHONE;
        }
        return audioDevice;
    }

    public String getDeviceNameFromValue(Integer value) {
        String name = null;
        if (value.equals(AudioOutput.EARSPEAKER.value)) {
            name = AudioOutput.EARSPEAKER.name;
        }
        if (value.equals(AudioOutput.LOUDSPEAKER.value)) {
            name = AudioOutput.LOUDSPEAKER.name;
        }
        if (value.equals(AudioOutput.HEADPHONES.value)) {
            name = AudioOutput.HEADPHONES.name;
        }
        if (value.equals(AudioOutput.BLUETOOTH.value)) {
            name = AudioOutput.BLUETOOTH.name;
    ...
    public interface AudioManagerEvents {
        void onAudioDeviceChanged(QBAudioManager.AudioDevice var1, Set<AudioDevice> var2);
    }
